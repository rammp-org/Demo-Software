...
class PerceptionCurbDescentDetectionNode(rclpy.node.Node):
    def __init__(self):
        super().__init__("perception_curb_descent_detection_node")
        self.get_logger().info(
            "Mock Perception Curb Descent Detection Node has been started."
        )
        self._mock_action_reject = False  # default to accept all goals
        self._mock_action_result = True  # default to successful execution
        self._action_running = False
    ...
    def _srv_detection_enable(self, request, response):
        if request.data:
            self.get_logger().info("Curb descent detection enabled.")
        else:
            self.get_logger().info("Curb descent detection disabled.")
        response.success = True
        return response
...
def main(args=None):
    ...
    node_descent = PerceptionCurbDescentDetectionNode()
    executor = MultiThreadedExecutor()
    executor.add_node(node)
    executor.add_node(node_descent)
