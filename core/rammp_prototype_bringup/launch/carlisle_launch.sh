...
tmux new-window -t "$SESSION" -n "jetson_mocks" \
    "zsh -c 'source $ROS_SETUP && \
            ...
            ros2 launch rammp_prototype_behavior carlisle_mock.launch.py; \
...
tmux new-window -t "$SESSION" -n "jetson" \
    "zsh -c 'source $ROS_SETUP && \
            source $JETSON_WS/install/setup.zsh && \
            ros2 launch rammp_prototype_bringup carlisle.launch.py $ARGS_STR > "rammp_logs/rammp_logs_$(date +%Y-%m-%d_%H-%M-%S).txt"; \
