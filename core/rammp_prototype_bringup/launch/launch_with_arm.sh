...
tmux new-session -d -s "$SESSION" -n "gui" \
    "bash -c 'cd $HOME && ./launch_ui.sh; echo \"[gui] exited.\"; read'"
...
tmux new-window -t "$SESSION" -n "jetson_mocks" \
    "zsh -c 'source $ROS_SETUP && \
            ...
            ros2 launch rammp_prototype_behavior mock_witharm.launch.py; \
...
tmux new-window -t "$SESSION" -n "jetson" \
    "zsh -c 'source $ROS_SETUP && \
            source $JETSON_WS/install/setup.zsh && \
            ros2 launch rammp_prototype_bringup full_witharm.launch.py $ARGS_STR > "rammp_logs/rammp_logs_$(date +%Y-%m-%d_%H-%M-%S).txt"; \
