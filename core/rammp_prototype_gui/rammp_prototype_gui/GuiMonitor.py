...
class GuiMonitor(Node):
    ...
    def __init__(self,shm_size=1024*1024):
        ...
        self.connection_publisher_timer = self.create_timer(1.0, self.publish_connection_status)
        ...
        self.manual_control_publisher = self.create_publisher(String, 'mebot/seat/manual_control', 10)

        # make subscriber for system state, message should be string
        self.system_state_subscriber = self.create_subscription(String, 'state', self.system_state_callback, 10)
            ...
            try:
                async with websockets.connect(uri) as ws:
                    ...
                    async for message in ws:
                        if self.ws_shutdown:
                            break
                        print(f"Received from Ethernet GUI: {message}")
