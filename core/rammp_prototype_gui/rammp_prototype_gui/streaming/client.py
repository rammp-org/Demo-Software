...
class StreamClient:
    ...
    def subscribe(
        self, channels: list[int] | None = None, compression: str = "none"
    ) -> None:
    ...
    def start(self) -> None:
        ...
        if self._sock is None:
            raise RuntimeError(
                "Cannot start receive thread: not connected. Call connect() first."
            )
        self._stopping.clear()
        self._thread = threading.Thread(
            target=self._recv_loop, daemon=True, name="rmss-recv"
        )
        self._thread.start()
            ...
            while True:
                ...
                if is_frame:
                    with self._stats_lock:
                        ...
                        if (
                            cs.last_seq >= 0
                            and msg.header.sequence_num > cs.last_seq + 1
                        ):
