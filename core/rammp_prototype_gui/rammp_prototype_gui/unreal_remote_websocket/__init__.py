...
class RemoteObjectProxy:
    ...
    def get_components(
        self, class_filter: Optional[str] = None
    ) -> list[RemoteObjectProxy]:
...
class UnrealRemote:
    ...
    def find_actors(
        self, class_filter: Optional[str] = None, name_filter: Optional[str] = None
    ) -> list[RemoteObjectProxy]:
        """
        ...
        try:
            if class_filter:
                ...
                result = self._call_function(self.RAMMS_CORE_BRIDGE, "GetAllActorPaths")
        ...
        try:
            result = self._call_function(
                self.EDITOR_ACTOR_SUBSYSTEM, "GetAllLevelActors"
            )
            ...
            if class_filter:
                actors = [
                    a for a in actors if class_filter.lower() in a.object_path.lower()
                ]
            return actors
        except UnrealRemoteError as e:
    ...
    def find_ramms_widgets(self, class_filter: str = "") -> list[RemoteObjectProxy]:
        ...
        if not self.ramms_ui_bridge:
            logger.warning(
                "find_ramms_widgets: No UI bridge configured (RammsUI plugin required)"
            )
            return []
        ...
        try:
            if class_filter:
                ...
                result = self._call_function(
                    self.ramms_ui_bridge, "GetAllRammsWidgetPaths"
                )
    ...
    def find_actors_by_component(self, component_class_filter: str) -> list[dict]:
        """
    ...
    def find_components(self, actor_path: str, class_filter: str = "") -> list[dict]:
        """
    ...
    def _get_components_via_describe(
        self, actor_path: str, class_filter: str = ""
    ) -> list[dict]:
    ...
    def _parse_path_list(
        self, result: Any, name_filter: Optional[str] = None
    ) -> list[RemoteObjectProxy]:
    ...
    def _parse_actor_list(
        ...
        for entry in paths:
            if isinstance(entry, str):
                ...
                path = entry.get(
                    "ObjectPath", entry.get("Path", entry.get("$ObjectPath", ""))
                )
            else:
                continue
    ...
    def _get_property(
        self, object_path: str, property_name: str, access: str = "READ_ACCESS"
    ) -> Any:
    ...
    def _set_property(self, object_path: str, property_name: str, value: Any) -> Any:
    ...
    def _call_function(
        self, object_path: str, function_name: str, parameters: Optional[dict] = None
    ) -> Any:
    ...
    def _request(self, method: str, endpoint: str, body: Optional[dict] = None) -> Any:
        ...
        try:
            with urlopen(req, timeout=self.timeout) as resp:
                ...
                if _debug:
                    logger.debug(
                        f"← {resp.status} {resp_data[:500] if resp_data else '(empty)'}"
                    )
                if resp_data:
            ...
            raise UnrealRemoteError(f"Connection failed to {url}: {e.reason}") from e
