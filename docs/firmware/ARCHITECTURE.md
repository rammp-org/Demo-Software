...
| 101–203 | `sendTelemetry()`        | ~50 consecutive `Serial.print()` calls; emits the telemetry packet                                                                              |
...
1. **Limit switch pin configuration** (Lines 320–324): `INPUT_PULLUP` mode for all 4 carriage limit switch pins.

1. **1-second delay** (Line 326): Allows hardware (especially the IMU) time to power-stabilize before initialization.

1. **IMU initialization** (Lines 328–334): `bno.begin()` over I²C. On failure, prints an error to Serial — but importantly, **does not halt** (unlike `IMU_Class::initialize_BNO055_sensor()` which blocks). The IMU being unavailable is recoverable — the system will operate but self-leveling will produce incorrect outputs.

1. **EEPROM load** (Lines 336–380): `ConfigStorage::begin()` validates the EEPROM magic number and initializes defaults if uninitialized. Then for each of the 6 motors:
...
1. **State transition to `IDLE`** (Line 382): Boot complete.
