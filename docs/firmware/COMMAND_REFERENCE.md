...
```
src/CommandParser/
├── CommandParser.h   (62 lines)  — CommandType enum, RobotCommand struct, class declaration
...
If `parser.isTimedOut()` returns `true`, `Base.ino` forcibly transitions to `ESTOP` (Line 415–418):
...
This ensures all motors cut power if serial communication is lost for over 60 seconds — for example, if the controlling PC crashes or the USB cable is disconnected.
