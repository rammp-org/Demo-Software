...
The Teensy needs to communicate 44 discrete float/int values back to the PC to keep the UI in sync. To prevent serial buffer overflow and maintain performance, Telemetry is strictly emitted at **10Hz** using a non-blocking millis() timer.
...
## `sendTelemetry()`
Located in `Base.ino`, this function executes 44 consecutive `Serial.print()` statements.
