...
ROS 2 node that maps a small **W / E / R / T** keypad to RAMMP high-level
...
interface 0 actually emits the keypresses. `udev/99-rammp-keyboard.rules` creates a
stable `/dev/rammp_keyboard` symlink to that node so the launch file can pin it
...
```bash
sudo cp hardware/keyboard_driver/udev/99-rammp-keyboard.rules /etc/udev/rules.d/
sudo udevadm control --reload-rules && sudo udevadm trigger
ls -l /dev/rammp_keyboard   # -> ../input/event5 (or wherever interface 0 lands)
```
...
| ------------- | --------------------- | --------------------------------------------------- |
| `device_path` | `/dev/rammp_keyboard` | Exact device/symlink to open. Empty => auto-select. |
