...
# ODrive setpoints use logical turns (Teensy TUNER_MODE o1:/o2:, y1:/y2:)
ODRIVE_POSITION_UNIT = "turns"
ODRIVE_VELOCITY_UNIT = "turns/s"
...
class ControlPanel(QWidget):
    ...
    def __init__(
        ...
        self._odrive_ui_joint: int | None = None
    ...
    def _send_odrive_velocity(self, target: float) -> None:
        """Engage TUNER_MODE velocity on one ODrive (y1: left, y2: right)."""
        joint_id = self._selected_joint_id()
        axis = odrive_axis_id(joint_id)
        self._data_store.set_target(joint_id, target)
        self._serial_handler.set_odrive_velocity(axis, target)

    def _send_odrive_target(self, target: float) -> None:
        if self._current_mode == MODE_VELOCITY:
            self._send_odrive_velocity(target)
        else:
            self._send_odrive_position(target)

    def _apply_odrive_unit_labels(self, mode: int = MODE_POSITION) -> None:
        if mode == MODE_VELOCITY:
            unit = ODRIVE_VELOCITY_UNIT
            indicator = "ODrive Velocity"
        else:
            unit = ODRIVE_POSITION_UNIT
            indicator = "ODrive Position"
            mode = MODE_POSITION
        ...
        self._mode_indicator_label.setText(indicator)
    ...
    def _setup_ui(self):
        ...
        self._seat_pos_group = self._create_seat_position_group()
        layout.addWidget(self._seat_pos_group)
        self._panels["Seat Position"] = self._seat_pos_group
    ...
    def _create_seat_position_group(self) -> CollapsibleGroupBox:
        group = CollapsibleGroupBox("Seat Position")
        content = QWidget()
        grid = QGridLayout(content)
        grid.setSpacing(SIZES["spacing_small"])
        grid.setColumnStretch(0, 1)
        ...
        self._seat_pos_paths: list[str | None] = [None] * len(self.SEAT_POS_SLOTS)
        self._seat_pos_choose_btns: list[QPushButton] = []
        self._seat_pos_play_btns: list[QPushButton] = []

        for i, label in enumerate(self.SEAT_POS_SLOTS):
            choose_btn = QPushButton(f"{label}: Empty")
            choose_btn.setStyleSheet(f"""
                QPushButton {{
                    background-color: {THEME.surface0};
                    color: {THEME.subtext0};
            ...
            choose_btn.clicked.connect(
                lambda _, idx=i: self._on_choose_seat_pos_sequence(idx)
            )
            grid.addWidget(choose_btn, i, 0)
            self._seat_pos_choose_btns.append(choose_btn)
            ...
            play_btn.setStyleSheet(f"""
                ...
                QPushButton:disabled {{ background-color: {THEME.surface1}; color: {THEME.overlay0}; }}
            """)
            play_btn.clicked.connect(lambda _, idx=i: self._on_seat_pos_play(idx))
            grid.addWidget(play_btn, i, 1)
            self._seat_pos_play_btns.append(play_btn)

        group.addWidget(content)
        return group

    def _on_choose_seat_pos_sequence(self, slot_idx: int):
        from PyQt6.QtWidgets import QFileDialog

        path, _ = QFileDialog.getOpenFileName(
            self,
            f"Choose Sequence for {self.SEAT_POS_SLOTS[slot_idx]}",
            "",
            "JSON Files (*.json);;All Files (*)",
            options=QFileDialog.Option.DontUseNativeDialog,
        )
        if not path:
            return
        self._seat_pos_paths[slot_idx] = path
        name = os.path.basename(path).replace(".json", "")
        btn = self._seat_pos_choose_btns[slot_idx]
        btn.setText(f"{self.SEAT_POS_SLOTS[slot_idx]}: {name}")
        btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {THEME.surface0};
                color: {THEME.text};
            ...
            QPushButton:pressed {{ background-color: {THEME.surface2}; }}
        """)
        self._seat_pos_play_btns[slot_idx].setEnabled(True)

    def _on_seat_pos_play(self, slot_idx: int):
        from PyQt6.QtWidgets import QMessageBox

        path = self._seat_pos_paths[slot_idx]
        if not path or not os.path.exists(path):
            return
    ...
    def _update_status(self):
        ...
        if self._is_odrive_selected():
            if self._odrive_ui_joint != joint_id:
                self._odrive_ui_joint = joint_id
                odrive_mode = (
                    self._current_mode
                    if self._current_mode in (MODE_POSITION, MODE_VELOCITY)
                    else MODE_POSITION
                )
                self._apply_odrive_unit_labels(odrive_mode)
        else:
            self._odrive_ui_joint = None
    ...
    def _on_set_target(self):
        ...
        if self._is_odrive_selected():
            self._send_odrive_target(target)
            return
    ...
    def _on_use_current(self):
        ...
        if self._is_odrive_selected():
            if self._current_mode == MODE_VELOCITY:
                self._target_input.setText(f"{joint_data.current_velocity:.4f}")
            else:
    ...
    def _on_set_mode(self, mode: int):
        ...
        if self._is_odrive_selected():
            ...
            if mode == MODE_POSITION:
                self._send_odrive_position(target)
                self._apply_odrive_unit_labels(MODE_POSITION)
            elif mode == MODE_VELOCITY:
                self._send_odrive_velocity(target)
                self._apply_odrive_unit_labels(MODE_VELOCITY)
            return
    ...
    def _on_step_complete(self):
        ...
        if self._is_odrive_selected():
            self._send_odrive_target(center)
            return
        joint_id = self._selected_joint_id()
    ...
    def _on_stop_sine(self):
        ...
        if self._is_odrive_selected():
            self._send_odrive_target(self._sine_center)
        else:
