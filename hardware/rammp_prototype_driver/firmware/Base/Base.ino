...
void updateTelemetry() {
    ...
    telemetry.rc_pos = rc.current_pos; telemetry.fc_pos = fc.current_pos;
    telemetry.ml_pos = ml.current_pos; telemetry.mr_pos = mr.current_pos;
    telemetry.ml_carriage_pos = ml_carriage.current_pos; telemetry.mr_carriage_pos = mr_carriage.current_pos;
    
    telemetry.rc_vel = rc.current_vel; telemetry.fc_vel = fc.current_vel;
    telemetry.ml_vel = ml.current_vel; telemetry.mr_vel = mr.current_vel;
    telemetry.ml_carriage_vel = ml_carriage.current_vel; telemetry.mr_carriage_vel = mr_carriage.current_vel;
}
...
void loop() {
    ...
    mr.updateSensorData(EContr.encoderf[5], 0.0f);
    ml_carriage.updateSensorData(EContr.encoderf[11], 0.0f);
    mr_carriage.updateSensorData(EContr.encoderf[12], 0.0f);
    ...
    if (current_state == TUNER_MODE && cmd.type != CMD_NONE) {
        ...
        switch(cmd.actuator_id) {
            ...
            case 2: m = &ml; break;
            case 3: m = &mr; break;
            case 4: m = &ml_carriage; break;
            case 5: m = &mr_carriage; break;
        }
    ...
    // Write PWM to RoboClaws (constrained strictly to 16-bit signed int +/- 32767)
    // roboclaw_main: M1 = Main Left, M2 = Main Right
    roboclaw_main.DutyM1(0x80, (int16_t)constrain(ml_pwm, -32767, 32767));
    roboclaw_main.DutyM2(0x80, (int16_t)constrain(mr_pwm, -32767, 32767));
    ...
    roboclaw_carriages.DutyM1(0x80, (int16_t)constrain(mlc_pwm, -32767, 32767));
    roboclaw_carriages.DutyM2(0x80, (int16_t)constrain(mrc_pwm, -32767, 32767));
