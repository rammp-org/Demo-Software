...
Carriage::Carriage(MotorID motor_id, int sw_1_pin, int sw_2_pin,
    ...
    : Component(NO_PIN, motor_id, fwd_is_positive), SW1_PIN(sw_1_pin),
      SW2_PIN(sw_2_pin), encoder_val(encoder) {
  des = 0.0;
};
...
void Carriage::calculate_carriages_position() {
  ...
  pos = 34.3 * (float)encoder_val /
        carriage_ticks; // 13.5" travel (ticks: carriage_ticks)
}
