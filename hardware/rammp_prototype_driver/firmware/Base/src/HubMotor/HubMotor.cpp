...
void HubMotor::writeTargetPos() {
  ...
  float position = getTargetPosition() * pos_scale * this->direction;

  uint32_t raw;

  memcpy(&raw, &position, 4);

  uint8_t payload[] = {0x4A, (uint8_t)(raw >> 24), (uint8_t)(raw >> 16),
...
void HubMotor::updateSensorData(float current_pos, float dt) {
  ...
  if (len == 10 && message[0] == 0xAA && message[2] == 0x57 &&
    ...
    float new_pos = position * this->direction;

    unsigned long sample_dt_ms =

        (last_pos_sample_ms > 0) ? (now - last_pos_sample_ms) : 0;

    last_pos_sample_ms = now;

    if (sample_dt_ms > 0) {

      float pos_dt = sample_dt_ms / 1000.0f;

      float raw_vel = (new_pos - this->prev_pos) / pos_dt;
