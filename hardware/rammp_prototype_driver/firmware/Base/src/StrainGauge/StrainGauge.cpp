...
StrainGauge::StrainGauge(int pin, float lpf_alpha)
    : pin(pin), lpf_alpha(lpf_alpha), _filtered_value(0.0f) {
  pinMode(pin, INPUT);
}
...
float StrainGauge::getValue() const { return _filtered_value; }
...
float StrainGauge::getLpfAlpha() const { return lpf_alpha; }
