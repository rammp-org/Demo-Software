...
// Serial.print Packet format (77 comma-separated values after the header):
//   TELEMETRY,<ms>,<state>,
...
//   <odrive L position>,<odrive R position>
void sendTelemetry() {
  char buf[800];
  int n = 0;
  ...
  n += snprintf(buf + n, sizeof(buf) - n, ",%.2f,%.2f",
                telemetry.odrive_l_position, telemetry.odrive_r_position);
